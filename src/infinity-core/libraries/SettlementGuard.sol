...
library SettlementGuard {
    /// @dev Storage slots (same as original for compatibility)
    ...
    function setLocker(address newLocker) internal {
        ...
        assembly ("memory-safe") {
            sstore(LOCKER_SLOT, and(newLocker, 0xffffffffffffffffffffffffffffffffffffffff))
        }
    }
    ...
    function getUnsettledDeltasCount() internal view returns (uint256 count) {
        assembly ("memory-safe") {
            count := sload(UNSETTLED_DELTAS_COUNT)
        }
    }
    ...
    function accountDelta(address settler, Currency currency, int256 newlyAddedDelta) internal {
        ...
        unchecked {
            if (nextDelta == 0) {
                assembly ("memory-safe") {
                    sstore(UNSETTLED_DELTAS_COUNT, sub(sload(UNSETTLED_DELTAS_COUNT), 1))
                }
            } else if (currentDelta == 0) {
                assembly ("memory-safe") {
                    sstore(UNSETTLED_DELTAS_COUNT, add(sload(UNSETTLED_DELTAS_COUNT), 1))
                }
            }
        }
